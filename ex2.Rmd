...
if(length(binned_coverage_nonzero) < 10) {
    ...
    legend("topright", legend=c("Observed Density", sprintf("Fitted Normal (μ=%.1f, σ=%.1f)", fit_mean, fit_sd), sprintf("Poisson (λ=%.1f)", observed_mean)),
           col=c("black", "red", "blue"), lwd=c(2,2,1), lty=c(1,2,1), cex=0.8, bty="n")
